...
namespace HardwareWarehouseManagementSystem
{
    class Program
    {
        ...
        private static void ProcessItems(CustomQueue<HardwareItem> hardwareItems)
        {
            while (hardwareItems.QueueLength > 0)
            {
                System.Threading.Thread.Sleep(3000);
                HardwareItem hardwareItem = hardwareItems.GetItem(); // Remove item from queue
            }
        }
